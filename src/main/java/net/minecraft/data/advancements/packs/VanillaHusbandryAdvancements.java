...
public class VanillaHusbandryAdvancements implements AdvancementSubProvider {
    ...
    public void generate(HolderLookup.Provider p_255680_, Consumer<AdvancementHolder> p_251389_) {
        ...
        Advancement.Builder.advancement()
            ...
            .addCriterion(
                "kill_axolotl_target", EffectsChangedTrigger.TriggerInstance.gotEffectsFrom(EntityPredicate.Builder.entity().of(EntityType.AXOLOTL))
            )
            .display(
        ...
        addTamedWolfVariants(Advancement.Builder.advancement(), p_255680_)
            .parent(advancementholder3)
            .display(
                Items.BONE,
                ...
                StartRidingTrigger.TriggerInstance.playerStartsRiding(
                    EntityPredicate.Builder.entity()
                        .vehicle(
                            EntityPredicate.Builder.entity()
                                .of(EntityType.BOAT)
                                .passenger(EntityPredicate.Builder.entity().of(EntityType.GOAT))
                        )
                )
            )
    ...
    public static AdvancementHolder createBreedAllAnimalsAdvancement(
        AdvancementHolder p_301269_, Consumer<AdvancementHolder> p_266923_, Stream<EntityType<?>> p_266961_, Stream<EntityType<?>> p_266751_
    ) {
    ...
    private static Advancement.Builder addLeashedFrogVariants(Advancement.Builder p_249739_) {
        BuiltInRegistries.FROG_VARIANT
            .holders()
            .forEach(
                p_325841_ -> p_249739_.addCriterion(
                        p_325841_.key().location().toString(),
                        PlayerInteractTrigger.TriggerInstance.itemUsedOnEntity(
                            ItemPredicate.Builder.item().of(Items.LEAD),
                            Optional.of(
                                EntityPredicate.wrap(
                                    EntityPredicate.Builder.entity().of(EntityType.FROG).subPredicate(EntitySubPredicates.frogVariant(p_325841_))
                                )
                            )
                        )
                    )
    ...
    private static Advancement.Builder addBreedable(Advancement.Builder p_266978_, Stream<EntityType<?>> p_267147_, Stream<EntityType<?>> p_267091_) {
        p_267147_.forEach(
            p_296350_ -> p_266978_.addCriterion(
                    EntityType.getKey((EntityType<?>)p_296350_).toString(),
                    BredAnimalsTrigger.TriggerInstance.bredAnimals(EntityPredicate.Builder.entity().of((EntityType<?>)p_296350_))
                )
        );
        p_267091_.forEach(
            p_296346_ -> p_266978_.addCriterion(
                    EntityType.getKey((EntityType<?>)p_296346_).toString(),
                    BredAnimalsTrigger.TriggerInstance.bredAnimals(
                        Optional.of(EntityPredicate.Builder.entity().of((EntityType<?>)p_296346_).build()),
                        Optional.of(EntityPredicate.Builder.entity().of((EntityType<?>)p_296346_).build()),
                        Optional.empty()
                    )
                )
    ...
    private static Advancement.Builder addFishBuckets(Advancement.Builder p_249285_) {
        for (Item item : FISH_BUCKETS) {
            p_249285_.addCriterion(
                ...
                FilledBucketTrigger.TriggerInstance.filledBucket(ItemPredicate.Builder.item().of(item))
            );
        }
