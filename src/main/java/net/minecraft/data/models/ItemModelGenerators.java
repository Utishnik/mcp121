...
public class ItemModelGenerators {
    ...
    private void generateFlatItem(Item p_125092_, String p_125093_, ModelTemplate p_125094_) {
        p_125094_.create(
            ModelLocationUtils.getModelLocation(p_125092_, p_125093_), TextureMapping.layer0(TextureMapping.getItemTexture(p_125092_, p_125093_)), this.output
        );
    }
    ...
    private void generateItemWithOverlay(Item p_328925_) {
        this.generateLayeredItem(ModelLocationUtils.getModelLocation(p_328925_), TextureMapping.getItemTexture(p_328925_), TextureMapping.getItemTexture(p_328925_, "_overlay"));
    }
    ...
    private JsonObject generateBaseArmorTrimTemplate(ResourceLocation p_266939_, Map<TextureSlot, ResourceLocation> p_267324_, Holder<ArmorMaterial> p_331857_) {
        ...
        for (ItemModelGenerators.TrimModelData itemmodelgenerators$trimmodeldata : GENERATED_TRIM_MODELS) {
            ...
            jsonobject1.addProperty("model", this.getItemModelForTrimMaterial(p_266939_, itemmodelgenerators$trimmodeldata.name(p_331857_)).toString());
    ...
    private void generateArmorTrims(ArmorItem p_267151_) {
        if (p_267151_.getType().hasTrims()) {
            ...
            if (p_267151_.getMaterial().is(ArmorMaterials.LEATHER)) {
                ModelTemplates.TWO_LAYERED_ITEM
                    .create(
                        resourcelocation,
                        TextureMapping.layered(resourcelocation1, resourcelocation2),
                        this.output,
                        (p_325900_, p_325901_) -> this.generateBaseArmorTrimTemplate(p_325900_, p_325901_, p_267151_.getMaterial())
                    );
            } else {
                ModelTemplates.FLAT_ITEM
                    .create(
                        ...
                        this.output,
                        (p_325903_, p_325904_) -> this.generateBaseArmorTrimTemplate(p_325903_, p_325904_, p_267151_.getMaterial())
                    );
            }
    ...
    static record TrimModelData(String name, float itemModelIndex, Map<Holder<ArmorMaterial>, String> overrideArmorMaterials) {
        public String name(Holder<ArmorMaterial> p_330580_) {
            return this.overrideArmorMaterials.getOrDefault(p_330580_, this.name);
        }
    }
}