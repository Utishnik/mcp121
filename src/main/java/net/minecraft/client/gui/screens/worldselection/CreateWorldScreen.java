...
public class CreateWorldScreen extends Screen {
    ...
    private final TabManager tabManager = new TabManager(this::addRenderableWidget, p_325424_ -> this.removeWidget(p_325424_));
    ...
    public static CreateWorldScreen createFromExisting(
        ...
        CreateWorldScreen createworldscreen = new CreateWorldScreen(
            p_276017_, p_276029_, p_276028_, WorldPresets.fromSettings(p_276028_.selectedDimensions()), OptionalLong.of(p_276028_.options().seed())
        );
    ...
    private CreateWorldScreen(
        Minecraft p_276053_, @Nullable Screen p_276049_, WorldCreationContext p_276047_, Optional<ResourceKey<WorldPreset>> p_276013_, OptionalLong p_276031_
    ) {
    ...
    private void onCreate() {
        ...
        WorldOpenFlows.confirmWorldCreation(
            this.minecraft, this, lifecycle2, () -> this.createNewWorld(worlddimensions$complete.specialWorldProperty(), layeredregistryaccess, lifecycle2), flag
        );
    }
    ...
    public void render(GuiGraphics p_282137_, int p_283640_, int p_281243_, float p_282743_) {
        ...
        p_282137_.blit(Screen.FOOTER_SEPARATOR, 0, this.height - this.layout.getFooterHeight() - 2, 0.0F, 0.0F, this.width, 2, 32, 2);
        RenderSystem.disableBlend();
    }

    @Override
    protected void renderMenuBackground(GuiGraphics p_334471_) {
        p_334471_.blit(TAB_HEADER_BACKGROUND, 0, 0, 0.0F, 0.0F, this.width, this.layout.getHeaderHeight(), 16, 16);
    ...
    private void applyNewPackConfig(PackRepository p_270272_, WorldDataConfiguration p_270573_, Consumer<WorldDataConfiguration> p_270552_) {
        ...
        WorldLoader.<CreateWorldScreen.DataPackReloadCookie, WorldCreationContext>load(
                worldloader$initconfig,
                p_325422_ -> {
                    if (p_325422_.datapackWorldgen().registryOrThrow(Registries.WORLD_PRESET).size() == 0) {
                        throw new IllegalStateException("Needs at least one world preset to continue");
                    } else if (p_325422_.datapackWorldgen().registryOrThrow(Registries.BIOME).size() == 0) {
                        ...
                        return new WorldLoader.DataLoadOutput<>(
                            new CreateWorldScreen.DataPackReloadCookie(worldgensettings, p_325422_.dataConfiguration()), p_325422_.datapackDimensions()
                        );
                    }
                },
        ...
        try {
            ...
            try {
                ...
                try (Stream<Path> stream = Files.walk(this.tempDataPackDir)) {
                    ...
                    stream.filter(p_232921_ -> !p_232921_.equals(this.tempDataPackDir)).forEach(p_232945_ -> copyBetweenDirs(this.tempDataPackDir, path, p_232945_));
