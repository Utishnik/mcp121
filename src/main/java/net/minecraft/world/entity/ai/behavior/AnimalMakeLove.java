...
public class AnimalMakeLove extends Behavior<Animal> {
    ...
    private Optional<? extends Animal> findValidBreedPartner(Animal p_22432_) {
        return p_22432_.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get().findClosest(p_341292_ -> {
            if (p_341292_.getType() == this.partnerType && p_341292_ instanceof Animal animal && p_22432_.canMate(animal) && !animal.isPanicking()) {
                return true;
            }
