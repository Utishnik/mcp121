...
public class DefaultRandomPos {
    ...
    public static Vec3 getPosAway(PathfinderMob p_148408_, int p_148409_, int p_148410_, Vec3 p_148411_) {
        ...
        return RandomPos.generateRandomPos(p_148408_, () -> {
            BlockPos blockpos = RandomPos.generateRandomDirectionWithinRadians(p_148408_.getRandom(), p_148409_, p_148410_, 0, vec3.x, vec3.z, (float) (Math.PI / 2));
    ...
    private static BlockPos generateRandomPosTowardDirection(PathfinderMob p_148437_, int p_148438_, boolean p_148439_, BlockPos p_148440_) {
        ...
        return !GoalUtils.isOutsideLimits(blockpos, p_148437_)
                && !GoalUtils.isRestricted(p_148439_, p_148437_, blockpos)
                && !GoalUtils.isNotStable(p_148437_.getNavigation(), blockpos)
                && !GoalUtils.hasMalus(p_148437_, blockpos)
            ? blockpos
            : null;
    }
}