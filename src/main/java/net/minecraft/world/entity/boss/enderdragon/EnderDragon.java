...
public class EnderDragon extends Mob implements Enemy {
    ...
    public void aiStep() {
        ...
        if (this.isDeadOrDying()) {
            ...
            this.level()
                .addParticle(
                    ParticleTypes.EXPLOSION, this.getX() + (double)f7, this.getY() + 2.0 + (double)f9, this.getZ() + (double)f10, 0.0, 0.0, 0.0
                );
        } else {
            this.checkCrystals();
            ...
            if (this.isNoAi()) {
                ...
                if (this.level() instanceof ServerLevel serverlevel1 && this.hurtTime == 0) {
                    this.knockBack(
                        serverlevel1,
                        serverlevel1.getEntities(this, this.wing1.getBoundingBox().inflate(4.0, 2.0, 4.0).move(0.0, -2.0, 0.0), EntitySelector.NO_CREATIVE_OR_SPECTATOR)
                    );
                    this.knockBack(
                        serverlevel1,
                        serverlevel1.getEntities(this, this.wing2.getBoundingBox().inflate(4.0, 2.0, 4.0).move(0.0, -2.0, 0.0), EntitySelector.NO_CREATIVE_OR_SPECTATOR)
                    );
