...
public class DragonSittingScanningPhase extends AbstractDragonSittingPhase {
    ...
    public DragonSittingScanningPhase(EnderDragon p_31342_) {
        super(p_31342_);
        this.scanTargeting = TargetingConditions.forCombat().range(20.0).selector(p_341435_ -> Math.abs(p_341435_.getY() - p_31342_.getY()) <= 10.0);
    }
        ...
        if (livingentity != null) {
            if (this.scanningTime > 25) {
                ...
                if (f1 < 0.0F || f1 > 10.0F) {
                    ...
                    double d2 = Mth.clamp(
                        Mth.wrapDegrees(180.0 - Mth.atan2(d0, d1) * 180.0F / (float)Math.PI - (double)this.dragon.getYRot()), -100.0, 100.0
                    );
            ...
            livingentity = this.dragon
                .level()
                .getNearestPlayer(CHARGE_TARGETING, this.dragon, this.dragon.getX(), this.dragon.getY(), this.dragon.getZ());
