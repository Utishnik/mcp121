...
public class PiglinAi {
    ...
    public static void angerNearbyPiglins(Player p_34874_, boolean p_34875_) {
        ...
        list.stream().filter(PiglinAi::isIdle).filter(p_34881_ -> !p_34875_ || BehaviorUtils.canSee(p_34881_, p_34874_)).forEach(p_341457_ -> {
            if (p_341457_.level().getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                setAngerTargetToNearestTargetablePlayerIfFound(p_341457_, p_34874_);
            } else {
                setAngerTarget(p_341457_, p_34874_);
            }
        });
    }
    ...
    protected static void wasHurtBy(Piglin p_34838_, LivingEntity p_34839_) {
        if (!(p_34839_ instanceof Piglin)) {
            ...
            getAvoidTarget(p_34838_).ifPresent(p_341460_ -> {
                if (p_341460_.getType() != p_34839_.getType()) {
    ...
    private static BehaviorControl<LivingEntity> babySometimesRideBabyHoglin() {
        ...
        return CopyMemoryWithExpiry.create(
            p_341455_ -> p_341455_.isBaby() && setentitylooktargetsometimes$ticker.tickDownAndCheck(p_341455_.level().random),
    ...
    protected static void broadcastAngerTarget(AbstractPiglin p_34896_, LivingEntity p_34897_) {
        getAdultPiglins(p_34896_).forEach(p_341453_ -> {
            if (p_34897_.getType() != EntityType.HOGLIN || p_341453_.canHunt() && ((Hoglin)p_34897_).canBeHunted()) {
                setAngerTargetIfCloserThanCurrent(p_341453_, p_34897_);
            }
        });
    }
