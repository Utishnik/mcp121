...
public class WorldBorderCommand {
                        ...
                        .then(
                            Commands.argument("distance", DoubleArgumentType.doubleArg(-5.999997E7F, 5.999997E7F))
                                .executes(
                                    p_326362_ -> setSize(
                                            p_326362_.getSource(),
                                            p_326362_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_326362_, "distance"),
                                            0L
                                        )
                                )
                                .then(
                                    Commands.argument("time", IntegerArgumentType.integer(0))
                                        .executes(
                                            p_326361_ -> setSize(
                                                    p_326361_.getSource(),
                                                    p_326361_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_326361_, "distance"),
                                                    p_326361_.getSource().getLevel().getWorldBorder().getLerpRemainingTime()
                                                        + (long)IntegerArgumentType.getInteger(p_326361_, "time") * 1000L
                                                )
                                        )
                                )
                        )
    ...
    private static int setCenter(CommandSourceStack p_139263_, Vec2 p_139264_) throws CommandSyntaxException {
        ...
        } else if (!((double)Math.abs(p_139264_.x) > 2.9999984E7) && !((double)Math.abs(p_139264_.y) > 2.9999984E7)) {
