...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider, GeneratingChunkMap {
    ...
    public ReportedException debugFuturesAndCreateReportedException(IllegalStateException p_203752_, String p_203753_) {
        ...
        Consumer<ChunkHolder> consumer = p_341211_ -> p_341211_.getAllFutures()
                .forEach(
                    p_341209_ -> {
                        ChunkStatus chunkstatus = p_341209_.getFirst();
                        CompletableFuture<ChunkResult<ChunkAccess>> completablefuture = p_341209_.getSecond();
                        if (completablefuture != null && completablefuture.isDone() && completablefuture.join() == null) {
                            stringbuilder.append(p_341211_.getPos())
                                .append(" - status: ")
                                .append(chunkstatus)
                                .append(" future: ")
    ...
    public CompletableFuture<ChunkResult<LevelChunk>> prepareEntityTickingChunk(ChunkHolder p_281455_) {
        return this.getChunkRangeFuture(p_281455_, 2, p_326375_ -> ChunkStatus.FULL)
            .thenApplyAsync(p_326417_ -> p_326417_.map(p_214939_ -> (LevelChunk)p_214939_.get(p_214939_.size() / 2)), this.mainThreadExecutor);
    }
    ...
    protected void saveAllChunks(boolean p_140319_) {
        if (p_140319_) {
            ...
            do {
                ...
                list.stream()
                    .map(p_341213_ -> {
                        this.mainThreadExecutor.managedBlock(p_341213_::isReadyForSaving);
                        return p_341213_.getLatestChunk();
                    })
    ...
    private void scheduleUnload(long p_140182_, ChunkHolder p_140183_) {
        ...
        }, this.unloadQueue::add).whenComplete((p_341217_, p_341218_) -> {
            if (p_341218_ != null) {
                LOGGER.error("Failed to save chunk {}", p_140183_.getPos(), p_341218_);
            }
        });
    }
    ...
    private CompletableFuture<ChunkAccess> scheduleChunkLoad(ChunkPos p_140418_) {
        return this.readChunk(p_140418_).thenApply(p_214925_ -> p_214925_.filter(p_214928_ -> {
                ...
                return flag;
            })).thenApplyAsync(p_341225_ -> {
            this.level.getProfiler().incrementCounter("chunkLoad");
            if (p_341225_.isPresent()) {
                ChunkAccess chunkaccess = ChunkSerializer.read(this.level, this.poiManager, this.storageInfo(), p_140418_, p_341225_.get());
                this.markPosition(p_140418_, chunkaccess.getPersistedStatus().getChunkType());
                return chunkaccess;
            } else {
                return this.createEmptyChunk(p_140418_);
            }
        }, this.mainThreadExecutor).exceptionallyAsync(p_326410_ -> this.handleChunkLoadFailure(p_326410_, p_140418_), this.mainThreadExecutor);
    }
    ...
    public CompletableFuture<ChunkResult<LevelChunk>> prepareTickingChunk(ChunkHolder p_143054_) {
        ...
        CompletableFuture<ChunkResult<LevelChunk>> completablefuture1 = completablefuture.<ChunkResult<LevelChunk>>thenApplyAsync(
                p_326406_ -> p_326406_.map(p_296573_ -> (LevelChunk)p_296573_.get(p_296573_.size() / 2)),
                p_341220_ -> this.mainThreadMailbox.tell(ChunkTaskPriorityQueueSorter.message(p_143054_, p_341220_))
            )
            .thenApplyAsync(p_326414_ -> p_326414_.ifSuccess(p_341227_ -> {
                    p_341227_.postProcessGeneration();
                    this.level.startTickingChunk(p_341227_);
                    CompletableFuture<?> completablefuture2 = p_143054_.getSendSyncFuture();
                    if (completablefuture2.isDone()) {
                        this.onChunkReadyToSend(p_341227_);
                    } else {
                        completablefuture2.thenAcceptAsync(p_296572_ -> this.onChunkReadyToSend(p_341227_), this.mainThreadExecutor);
                    }
    ...
    public CompletableFuture<ChunkResult<LevelChunk>> prepareAccessibleChunk(ChunkHolder p_143110_) {
        return this.getChunkRangeFuture(p_143110_, 1, ChunkLevel::getStatusAroundFullChunk)
            .thenApplyAsync(
                p_326418_ -> p_326418_.map(p_203092_ -> (LevelChunk)p_203092_.get(p_203092_.size() / 2)),
                p_341231_ -> this.mainThreadMailbox.tell(ChunkTaskPriorityQueueSorter.message(p_143110_, p_341231_))
            );
    }
